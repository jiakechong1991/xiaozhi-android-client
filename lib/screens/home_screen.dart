...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Scaffold(
        ...
        // 顶部导航栏
        appBar: _buildTitleBar(_selectedIndex),
    ...
    if (selectedIndexT == 0) {
      ...
      return SafeArea(
        ...
        child: Column(
          children: [
            _buildSearchBar(),
            Expanded(child: _buildAgentRoleListPageBody()),
          ],
        ),
      );
    } else {
  ...
  AppBar? _buildTitleBar(int selectedIndexT) {
    if (selectedIndexT == 0) {
      //消息页面
      return AppBar(
  ...
  ////// build 演员agent列表
  Widget _buildAgentRoleListPageBody() {
    return Obx(
      () => ListView(
        // 滚动容器
        padding: EdgeInsets.only(
